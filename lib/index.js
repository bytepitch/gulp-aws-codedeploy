...
function codeDeploy (codedeployParams, s3Root) {
  ...
  console.log('** codedeployParams: ', codedeployParams);
  ...
  return through.obj(function (file, enc, cb) {
    ...
    const s3Stream = s3fs.createWriteStream(targetFilename, function (err, result) {
      ...
      if (err) {
        return emitError(this, 'Could not upload ' + file.path + ' to ' + targetFilename, cb);
      }
    ...
    // pipe file
    if ( file.isStream() ) {
      file.contents.pipe( s3Stream );
    } else if ( file.isBuffer() ) {
      s3Stream.end( file.contents );
    }
  });
}
